...
from utils import (
    ...
    build_output_tables,
    prefix_match, encode_data, exact_match
)
...
def setup_model(args, device, vocab_size, instruction_len, num_actions, num_targets):
    ...
    decoder = Decoder(device, embedding_dim, hidden_dim, num_layers, num_actions, num_targets).to(device)
    model = EncoderDecoder(device, encoder, decoder, num_actions, num_targets, hidden_dim).to(device)
    return model


def setup_optimizer(args, device, model):
    """
    return:
        - criterion: loss_fn
    ...
    # ===================================================== #
    learning_rate = 1e-4
    action_criterion = torch.nn.CrossEntropyLoss(ignore_index=0).to(device)
    target_criterion = torch.nn.CrossEntropyLoss(ignore_index=0).to(device)
...
def train_epoch(
    ...
    for (inputs, labels) in loader:
        ...
        action_outputs, target_outputs = model(inputs, labels, teacher_forcing=True)
        ...
        action_loss = action_criterion(action_outputs.squeeze().permute(0, 2, 1).to(device), labels[:, :, 0].long())
        target_loss = target_criterion(target_outputs.squeeze().permute(0, 2, 1).to(device), labels[:, :, 1].long())
        ...
        loss = action_loss + 100 * target_loss
        ...
        # action_em = predicted_action_outputs == labels[:, :, 0]
        # target_em = predicted_target_outputs == labels[:, :, 0]
...
def train(args, model, loaders, optimizer, action_criterion, target_criterion, device):
    ...
    for epoch in tqdm.tqdm(range(args.num_epochs)):
        ...
        train_action_loss, train_target_loss, train_action_prefix_acc, train_target_prefix_acc, \
        train_action_exact_acc, train_target_exact_acc = train_epoch(
            args,
            model,
            loaders["train"],
            optimizer,
